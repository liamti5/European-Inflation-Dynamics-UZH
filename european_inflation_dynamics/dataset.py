...
def data_long_lat():
    df_european_countries = pd.read_feather(PROCESSED_DATA_DIR / "countries-with-inflation-data.ftr").rename(columns={"Reference area": "name"})
...
def main():
    ...
    data_functions = [(name, func) for name, func in globals().items() if callable(func) and name.startswith('data_')]
